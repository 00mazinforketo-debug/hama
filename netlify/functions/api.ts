...
const buildFixedCatalog = () => {
  ...
  const categoryNamesById = new Map(categories.map((category) => [category.id, clone(category.names)] as const));
...
const sortCatalogRecords = (state: Pick<PersistedState, "categories" | "menuItems">) => {
  state.categories.sort((left, right) => left.sortOrder - right.sortOrder || left.slug.localeCompare(right.slug));
  state.menuItems.sort((left, right) => left.sortOrder - right.sortOrder || left.slug.localeCompare(right.slug));
};

const syncCatalogDefaults = (state: PersistedState) => {
  const fixedCatalog = buildFixedCatalog();
  let didChange = false;

  const categoriesById = new Map(state.categories.map((category) => [category.id, category] as const));
  for (const fixedCategory of fixedCatalog.categories) {
    const existingCategory = categoriesById.get(fixedCategory.id);

    if (!existingCategory) {
      state.categories.push(clone(fixedCategory));
      didChange = true;
      continue;
    }

    if (!existingCategory.slug) {
      existingCategory.slug = fixedCategory.slug;
      didChange = true;
    }

    if (!existingCategory.names?.ku) {
      existingCategory.names = clone(fixedCategory.names);
      didChange = true;
    }

    if (typeof existingCategory.sortOrder !== "number" || Number.isNaN(existingCategory.sortOrder)) {
      existingCategory.sortOrder = fixedCategory.sortOrder;
      didChange = true;
    }

    if (existingCategory.icon === undefined) {
      existingCategory.icon = fixedCategory.icon ?? null;
      didChange = true;
    }
  }

  const categoryNamesById = new Map(state.categories.map((category) => [category.id, clone(category.names)] as const));
  const menuItemsById = new Map(state.menuItems.map((item) => [item.id, item] as const));

  for (const fixedItem of fixedCatalog.menuItems) {
    const existingItem = menuItemsById.get(fixedItem.id);

    if (!existingItem) {
      state.menuItems.push({
        ...clone(fixedItem),
        categoryNames: clone(categoryNamesById.get(fixedItem.categoryId) ?? fixedItem.categoryNames)
      });
      didChange = true;
      continue;
    }
    ...
    if (!existingItem.slug) {
      existingItem.slug = fixedItem.slug;
      didChange = true;
    }

    if (!existingItem.categoryId || !categoryNamesById.has(existingItem.categoryId)) {
      existingItem.categoryId = fixedItem.categoryId;
      didChange = true;
    }

    const desiredCategoryNames = clone(categoryNamesById.get(existingItem.categoryId) ?? categoryNamesById.get(fixedItem.categoryId) ?? fixedItem.categoryNames);
    if (JSON.stringify(existingItem.categoryNames) !== JSON.stringify(desiredCategoryNames)) {
      existingItem.categoryNames = desiredCategoryNames;
      didChange = true;
    }

    if (typeof existingItem.basePrice !== "number" || Number.isNaN(existingItem.basePrice)) {
      existingItem.basePrice = fixedItem.basePrice;
      didChange = true;
    }

    if (typeof existingItem.sortOrder !== "number" || Number.isNaN(existingItem.sortOrder)) {
      existingItem.sortOrder = fixedItem.sortOrder;
      didChange = true;
    }

    if (typeof existingItem.isAvailable !== "boolean") {
      existingItem.isAvailable = fixedItem.isAvailable;
      didChange = true;
    }

    if (!existingItem.createdAt) {
      existingItem.createdAt = fixedItem.createdAt;
      didChange = true;
    }

    if (!Array.isArray(existingItem.translations) || !existingItem.translations.length) {
      existingItem.translations = clone(fixedItem.translations);
      didChange = true;
    }

    if (existingItem.imageUrl === undefined) {
      existingItem.imageUrl = fixedItem.imageUrl ?? null;
      didChange = true;
    }

    if (existingItem.imagePublicId === undefined) {
      existingItem.imagePublicId = fixedItem.imagePublicId ?? null;
      didChange = true;
    }
  }

  for (const item of state.menuItems) {
    const categoryNames = categoryNamesById.get(item.categoryId);
    if (categoryNames && JSON.stringify(item.categoryNames) !== JSON.stringify(categoryNames)) {
      item.categoryNames = clone(categoryNames);
      didChange = true;
    }
  }

  if (didChange) {
    sortCatalogRecords(state);
  }
...
const buildInitialState = (): PersistedState => {
  const { categories, menuItems } = buildFixedCatalog();
...
const createResilientStore = (
  ...
  const runWithFallback = async <T>(operation: () => Promise<T>, fallbackOperation: () => Promise<T>) => {
    ...
    try {
      ...
      if (!allowFallback) {
        console.error("[netlify-api] hosted blob operation failed", error);
        throw new ApiError("Persistent storage is unavailable.", 500);
      }
...
const normalizeLambdaHeaders = (headers: HandlerEvent["headers"]): Record<string, string> => {
...
const getBlobStore = (event: HandlerEvent): PersistedStore => {
  ...
  const allowFallback = !isHostedNetlifyEvent(event);
  ...
  try {
    // Shared state is updated on every order write, so hosted reads must be
    // strongly consistent or the admin dashboard can miss freshly submitted
    // orders from another request/edge location.
    return createResilientStore(
      getStore({ name: appStoreName, consistency: "strong" }),
      { allowFallback }
    );
  } catch (error) {
...
const loadState = async (store: PersistedStore) => {
  ...
  const didSyncCatalog = syncCatalogDefaults(stored);
  if (didSyncUsers || didPurgeSessions || didSyncCatalog) {
...
const activityWindowMs = 1000 * 60 * 60 * 24;

const getOrderActivityLast24Hours = (state: PersistedState, now: Date): AdminSummary["orderActivityLast24Hours"] => {
  const nowTime = now.getTime();
  const cutoffTime = nowTime - activityWindowMs;
  const usersById = new Map(state.users.map((user) => [user.id, user] as const));
  const activityByActor = new Map<string, AdminSummary["orderActivityLast24Hours"][number]>();

  for (const order of state.orders) {
    const placedAtTime = new Date(order.placedAt).getTime();
    if (!Number.isFinite(placedAtTime) || placedAtTime < cutoffTime || placedAtTime > nowTime) {
      continue;
    }

    const matchingUser = (order.submittedByUserId ? usersById.get(order.submittedByUserId) : null) ?? state.users.find((entry) => entry.displayName === order.submittedByName) ?? null;
    const actorName = matchingUser?.displayName ?? order.submittedByName ?? "ناسراو";
    const actorRole = matchingUser?.role ?? "CUSTOMER";
    const actorKey = matchingUser?.id ?? order.submittedByUserId ?? `name:${actorName}`;
    const current = activityByActor.get(actorKey) ?? {
      userId: matchingUser?.id ?? order.submittedByUserId ?? null,
      actorName,
      actorRole,
      orderCount: 0,
      lastOrderAt: null
    ...
    if (!current.lastOrderAt || placedAtTime > new Date(current.lastOrderAt).getTime()) {
      current.lastOrderAt = order.placedAt;
    }

    activityByActor.set(actorKey, current);
  }

  return [...activityByActor.values()].sort((left, right) => {
    const orderCountDelta = right.orderCount - left.orderCount;
    if (orderCountDelta !== 0) {
      return orderCountDelta;
    }

    const rightLastOrder = right.lastOrderAt ? new Date(right.lastOrderAt).getTime() : 0;
    const leftLastOrder = left.lastOrderAt ? new Date(left.lastOrderAt).getTime() : 0;
...
const getSummary = (state: PersistedState): AdminSummary => {
  ...
  return {
    ...
    orderActivityLast24Hours: getOrderActivityLast24Hours(state, now)
  };
};
