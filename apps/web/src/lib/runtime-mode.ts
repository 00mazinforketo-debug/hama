...
const configuredInfoUrl = import.meta.env.VITE_INFO_URL?.trim() || undefined;
...
export const getExternalInfoUrl = () => {
  if (!configuredInfoUrl) {
    if (typeof window === "undefined") {
      return "/";
    }
  ...
  try {
    ...
    return new URL(configuredInfoUrl, baseUrl).toString();
  } catch {
    return configuredInfoUrl;
  }
};
