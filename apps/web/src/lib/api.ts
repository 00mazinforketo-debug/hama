...
const shouldBypassHostedGetCaches = (method: string) => {
  if (typeof window === "undefined") {
    return false;
  }

  return !isLocalDevHost(window.location.hostname) && (method === "GET" || method === "HEAD");
};

const buildRequestUrl = (apiBase: string, path: string, method: string) => {
  const requestUrl = new URL(`${apiBase}${path}`);

  if (shouldBypassHostedGetCaches(method)) {
    // Live order/admin views should never reuse a stale cached GET response.
...
const sendWithBase = async <T>(apiBase: string, path: string, init?: RequestInit) => {
  ...
  const response = await fetch(buildRequestUrl(apiBase, path, method), {
    cache: "no-store",
    credentials: "include",
    ...init,
    headers
...
const download = async (path: string, filename: string) => {
  ...
  const sendDownload = () =>
    fetch(buildRequestUrl(API_URL, path, "GET"), {
      cache: "no-store",
      credentials: "include",
      headers: {
