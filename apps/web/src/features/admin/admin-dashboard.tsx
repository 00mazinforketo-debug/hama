...
const actorRoleLabels: Record<"CUSTOMER" | "ADMIN", string> = {
  CUSTOMER: "کڕیار",
  ADMIN: "ئادمین"
};
...
const AdminStatePanel = ({ title, message, action }: { title: string; message: string; action?: ReactNode }) => (
...
export const AdminDashboard = () => {
  ...
  const updateAdminRoute = (
    ...
    const nextUrl = nextSearch ? `/admin?${nextSearch}` : "/admin";
  ...
  const orderActivityLast24Hours = summaryQuery.data?.orderActivityLast24Hours ?? [];
  const hasPersistentStorageOutage =
    isPersistentStorageOutageError(summaryQuery.error) ||
    isPersistentStorageOutageError(revenueRangeQuery.error) ||
    isPersistentStorageOutageError(ordersQuery.error) ||
    isPersistentStorageOutageError(menuItemsQuery.error) ||
    isPersistentStorageOutageError(categoriesQuery.error) ||
    isPersistentStorageOutageError(usersQuery.error) ||
    isPersistentStorageOutageError(activityQuery.error);
  const criticalAdminQueryError =
    summaryQuery.error ??
    revenueRangeQuery.error ??
    ordersQuery.error ??
  ...
  if (hasPersistentStorageOutage) {
    return (
      <AdminStatePanel
        title="داشبۆردی ئادمین کاتێکی کورت وەستاوە"
        message="هەڵگرتنی زانیارییەکان لە production بەردەست نییە، بۆیە ئۆردەر، بەکارهێنەر و داتا ئێستا بارنابن."
      />
    );
  }

  if (criticalAdminQueryError) {
    return (
      <AdminStatePanel
        title="بارکردنی داشبۆرد سەرکەوتوو نەبوو"
        message={getQueryErrorMessage(criticalAdminQueryError, "ناتوانرێت داتای سەرەکیی ئادمین ئێستا باربکرێت.")}
        action={(
  ...
  const renderOrderActivityLast24Hours = (limit?: number) => {
    const visibleActivity = limit ? orderActivityLast24Hours.slice(0, limit) : orderActivityLast24Hours;

    if (summaryQuery.isLoading && !visibleActivity.length) {
      return (
    ...
    if (!visibleActivity.length) {
      return (
        <div className="rounded-[24px] border border-dashed border-white/10 bg-slate-950/30 p-8 text-center text-sm text-slate-300">
          <Users2 className="mx-auto mb-3 h-8 w-8 text-slate-500" />
          هیچ چاڵاکییەکی ئۆردەر لە 24 کاتژمێری دواییدا تۆمار نەکراوە.
        </div>
      );
    }
    ...
    return (
      <div className="space-y-3">
        {visibleActivity.map((entry) => (
          <div key={`${entry.userId ?? entry.actorName}-${entry.lastOrderAt ?? "none"}`} className="rounded-2xl border border-white/10 bg-slate-950/35 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="font-semibold text-white">{entry.actorName}</p>
                <p className="mt-1 text-xs text-slate-400">{actorRoleLabels[entry.actorRole]}</p>
              </div>
              <div className="text-left sm:text-right">
                <p className="font-display text-2xl font-extrabold text-amber-200">{toEnglishDigits(formatNumber(entry.orderCount))}</p>
                <p className="text-xs text-slate-400">ئۆردەر لە 24 کاتژمێری دوایی</p>
              </div>
            </div>
            {entry.lastOrderAt ? <p className="mt-2 text-xs text-slate-400">دوایین داواکاری: {formatDateTime(entry.lastOrderAt)}</p> : null}
          </div>
        ))}
      </div>
    );
  ...
  const renderRecentActivity = (limit?: number) => {
    const visibleActivity = limit ? (activityQuery.data ?? []).slice(0, limit) : (activityQuery.data ?? []);

    if (activityQuery.isLoading && !visibleActivity.length) {
      return (
        <div className="rounded-[24px] border border-dashed border-white/10 bg-slate-950/30 p-8 text-center text-sm text-slate-300">
          {t("common.loading")}
        </div>
      ...
      <div className="space-y-3">
        {visibleActivity.map((entry) => (
          <div key={entry.id} className="rounded-2xl border border-white/10 bg-slate-950/35 p-4">
            ...
            <p className="mt-2 text-sm text-slate-300">{toEnglishDigits(entry.action)} • {toEnglishDigits(entry.entityType)}</p>
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div className="app-shell">
      <div className="app-stage-admin">
        <div className="grid gap-4 xl:grid-cols-[16rem_minmax(0,1fr)] 2xl:gap-6">
          <aside className="min-w-0 space-y-4 xl:sticky xl:top-4 xl:self-start">
            <nav className="no-scrollbar flex gap-2 overflow-x-auto rounded-[20px] border border-white/10 bg-white/5 p-2 backdrop-blur-xl xl:flex-col xl:overflow-visible sm:rounded-[28px] sm:p-3">
              {visibleTabs.map((entry) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "inline-flex shrink-0 items-center gap-3 rounded-[18px] px-3.5 py-2.5 text-[0.85rem] font-semibold transition xl:w-full sm:rounded-2xl sm:px-4 sm:py-3 sm:text-sm",
                      tab === entry
          ...
          <main className="min-w-0 space-y-4 sm:space-y-6">
            ...
            {tab === "home" ? (
              <div className="grid gap-4 xl:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)] 2xl:gap-6">
                <Section title={t("nav.home")} icon={House}>
                  <div className="grid gap-3 min-[460px]:grid-cols-2 xl:grid-cols-3">
                    {tabs
                ...
                <div className="space-y-4">
                  <Section title="چاڵاکیی داواکاری لە 24 کاتژمێری دوایی" icon={Users2}>
                    {renderOrderActivityLast24Hours(5)}
                  </Section>

                  <Section title={t("common.recentActivity")} icon={BarChart3}>
                    {renderRecentActivity(5)}
                  </Section>
                </div>
              </div>
            ) : null}
            ...
            {tab === "orders" ? (
              <>
                <div className={cn("space-y-4 2xl:hidden", mobileOrdersView === "detail" ? "hidden" : "block")}>
                ...
                <div className={cn("space-y-4 2xl:hidden", mobileOrdersView === "detail" ? "block" : "hidden")}>
                ...
                <div className="hidden gap-4 2xl:grid 2xl:grid-cols-[22rem_minmax(0,1fr)] 2xl:gap-6">
                  <Section title={t("admin.ordersBoard")} icon={ClipboardList}>
                    <div className="space-y-4">
                      <input value={orderSearch} onChange={(event) => setOrderSearch(event.target.value)} placeholder={t("admin.orderSearchHint")} className="compact-input" />
                      <div className="grid gap-3">
        ...
        {tab === "menu" ? (
          <div className="grid gap-4 xl:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] 2xl:gap-6">
        ...
        {tab === "categories" ? (
          <div className="grid gap-4 xl:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)] 2xl:gap-6">
        ...
        {tab === "users" ? (
          <div className="grid gap-4 xl:grid-cols-[minmax(0,0.76fr)_minmax(0,1.24fr)] 2xl:gap-6">
        ...
        {tab === "activity" ? (
          <div className="grid gap-4 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] 2xl:gap-6">
            <Section title="چاڵاکیی داواکاری لە 24 کاتژمێری دوایی" icon={Users2}>
              {renderOrderActivityLast24Hours()}
            </Section>
        ...
        {tab === "about" ? (
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] 2xl:gap-6">
...
const AdminCategoryEditorForm = ({
  ...
  <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(18rem,0.8fr)] 2xl:gap-6">
...
const AdminMenuEditorForm = ({
  ...
  <div className="grid gap-4 xl:grid-cols-[minmax(0,1.05fr)_minmax(18rem,0.95fr)] 2xl:gap-6">
...
export const AdminCategoryEditorPage = () => {
  ...
  if (isPersistentStorageOutageError(categoriesQuery.error)) {
    return <AdminStatePanel title="پەیجی پۆل بەردەست نییە" message="هەڵگرتنی داتای پۆلەکان لە production کێشەی هەیە." />;
  }

  if (categoriesQuery.error) {
    return (
      <AdminStatePanel
        title="بارکردنی پۆلەکان سەرکەوتوو نەبوو"
        message={getQueryErrorMessage(categoriesQuery.error, "ناتوانرێت داتای پۆلەکان ئێستا باربکرێت.")}
        action={(
          <button type="button" onClick={() => window.location.reload()} className="compact-pill-button justify-center">
            نوێکردنەوە
          </button>
...
export const AdminMenuItemEditorPage = () => {
  ...
  if (isPersistentStorageOutageError(menuItemsQuery.error) || isPersistentStorageOutageError(categoriesQuery.error)) {
    return <AdminStatePanel title="پەیجی خواردن بەردەست نییە" message="هەڵگرتنی داتای خواردن و پۆلەکان لە production کێشەی هەیە." />;
  }

  if (menuItemsQuery.error || categoriesQuery.error) {
    return (
      <AdminStatePanel
        title="بارکردنی خواردنەکان سەرکەوتوو نەبوو"
        message={getQueryErrorMessage(menuItemsQuery.error ?? categoriesQuery.error, "ناتوانرێت داتای خواردن و پۆلەکان ئێستا باربکرێت.")}
        action={(
          <button type="button" onClick={() => window.location.reload()} className="compact-pill-button justify-center">
            نوێکردنەوە
          </button>
