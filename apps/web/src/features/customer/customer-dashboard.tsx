...
import { getExternalInfoUrl } from "../../lib/runtime-mode";
...
const externalInfoHref = getExternalInfoUrl();
...
type CustomerAvailabilityCatalog = {
  categories: Pick<CategoryDto, "id">[];
...
const sanitizeAvailabilityEntries = (current: Record<string, boolean>, validIds: Set<string>) => {
  const next: Record<string, boolean> = {};

  for (const [id, enabled] of Object.entries(current)) {
    if (typeof enabled === "boolean" && validIds.has(id)) {
      next[id] = enabled;
    }
  }
...
const areAvailabilityMapsEqual = (left: Record<string, boolean>, right: Record<string, boolean>) => {
  const leftEntries = Object.entries(left);
  if (leftEntries.length !== Object.keys(right).length) {
    return false;
  }

  return leftEntries.every(([id, enabled]) => right[id] === enabled);
};

const areAvailabilitySettingsEqual = (left: CustomerAvailabilitySettings, right: CustomerAvailabilitySettings) =>
  areAvailabilityMapsEqual(left.categories, right.categories) && areAvailabilityMapsEqual(left.menuItems, right.menuItems);

const sanitizeCustomerAvailabilitySettings = (
  settings: CustomerAvailabilitySettings,
  catalog: CustomerAvailabilityCatalog
): CustomerAvailabilitySettings => {
  const categoryIds = new Set(catalog.categories.map((category) => category.id));
  const menuItemIds = new Set(catalog.menuItems.map((item) => item.id));

  return {
    categories: sanitizeAvailabilityEntries(settings.categories, categoryIds),
    menuItems: sanitizeAvailabilityEntries(settings.menuItems, menuItemIds)
  };
};

const hasAnyVisibleCustomerCatalogEntries = (
  settings: CustomerAvailabilitySettings,
  catalog: CustomerAvailabilityCatalog
) => {
  if (!catalog.categories.length || !catalog.menuItems.length) {
    return true;
  }

  return catalog.categories.some((category) => {
    if (!resolveCategoryAvailability(settings, category.id)) {
      return false;
    }

    return catalog.menuItems.some((item) => item.categoryId === category.id && resolveMenuItemAvailability(settings, item));
  });
};
...
const useCustomerAvailabilitySettings = (catalog?: CustomerAvailabilityCatalog) => {
  const { user } = useAuth();
  ...
  const catalogMenuItems = catalog?.menuItems;
  ...
  useEffect(() => {
    if (!user?.id || !catalogCategories?.length || !catalogMenuItems?.length) {
      return;
    }
    ...
    } satisfies CustomerAvailabilityCatalog;
    const sanitized = sanitizeCustomerAvailabilitySettings(settings, nextCatalog);
    const shouldResetAll = !hasAnyVisibleCustomerCatalogEntries(sanitized, nextCatalog);
    const next = shouldResetAll ? createDefaultAvailabilitySettings() : sanitized;

    if (areAvailabilitySettingsEqual(settings, next)) {
      return;
    }
    ...
    if (shouldResetAll) {
      toast.success("هەموو پۆلەکان و خواردنەکان گەڕێندرایەوە.");
    }
  }, [catalogCategories, catalogMenuItems, settings, user?.id]);

  const updateSettings = (updater: (current: CustomerAvailabilitySettings) => CustomerAvailabilitySettings) => {
    setSettings((current) => {
      const next = updater(current);
      if (areAvailabilitySettingsEqual(current, next)) {
        return current;
      }
  ...
  const resetAllAvailability = () => {
    const next = createDefaultAvailabilitySettings();
    if (areAvailabilitySettingsEqual(settings, next)) {
      return;
    }
...
export const CustomerDashboard = () => {
  ...
  const emptyCartRouteOverrideRef = useRef<MobileCustomerView | null>(null);
  const postSubmitRedirectRef = useRef<MobileCustomerView | null>(null);
  ...
  const updateCustomerRootRoute = (nextView: MobileCustomerView | "dashboard", replace = false) => {
    ...
    const nextUrl = nextSearch ? `/app?${nextSearch}` : "/app";
  ...
  const syncCustomerOrdersOverview = (replace = false) => {
    setNotificationCenterOpen(false);
    setMobileView("orders");
    setDesktopView("orders");
    updateCustomerRootRoute("orders", replace);
  ...
  useEffect(() => {
    if (!lines.length) {
      ...
      if (mobileView === "cart" || mobileView === "checkout") {
        const nextView = postSubmitRedirectRef.current ?? emptyCartRouteOverrideRef.current ?? "browse";
        emptyCartRouteOverrideRef.current = null;
        setMobileView(nextView);
        updateCustomerRootRoute(nextView, true);
      }
      return;
    }
  ...
  useEffect(() => {
    if (postSubmitRedirectRef.current !== "orders") {
      return;
    }

    if (requestedRootView === "orders" && mobileView === "orders" && desktopView === "orders") {
      postSubmitRedirectRef.current = null;
      return;
    }

    syncCustomerOrdersOverview(true);
  }, [desktopView, mobileView, requestedRootView]);
  ...
  const criticalCustomerQueryError =
    menuQuery.error ??
    orderMenuItemsQuery.error ??
    currentOrdersQuery.error ??
    historyQuery.error ??
    settingsCatalogQuery.catalogQuery.error ??
    null;
  ...
  const orderMutation = useMutation({
    ...
    onSuccess: () => {
      postSubmitRedirectRef.current = "orders";
      emptyCartRouteOverrideRef.current = "orders";
      setCheckoutStep("cart");
      clear();
      setCustomerNameKu("");
      setCustomerPhone("");
  ...
  const customerAvailability = useCustomerAvailabilitySettings({ categories: allCatalogCategories, menuItems: allCatalogMenuItems });
  ...
  const isSubmitDisabled = !lines.length || orderMutation.isPending;
  ...
  if (criticalCustomerQueryError) {
    return (
      <HostedMaintenancePanel
        title="بارکردنی زانیارییەکان سەرکەوتوو نەبوو"
        message={getQueryErrorMessage(criticalCustomerQueryError, "ناتوانرێت مێنیو، ئۆردەر یان ڕێکخستنەکان ئێستا باربکرێن. تکایە site نوێ بکەرەوە و دووبارە هەوڵ بدە.")}
        action={
          <button type="button" onClick={() => window.location.reload()} className="compact-pill-button justify-center">
            نوێکردنەوە
          </button>
        }
      />
  ...
  const focusCustomerOrdersOverview = (replace = false) => {
    syncCustomerOrdersOverview(replace);
  };

  const openMobileOrders = () => {
    focusCustomerOrdersOverview();
  };
  ...
  return (
    <div className="app-shell">
      ...
      <div className="app-stage space-y-3 sm:space-y-6">
          ...
          {mobileView === "checkout" ? (
            <section className="space-y-3 pb-6">
              ...
              <form className="space-y-3" onSubmit={placeOrder}>
                ...
                <button type="submit" disabled={isSubmitDisabled} className="w-full touch-manipulation rounded-[20px] bg-gradient-to-r from-amber-300 via-orange-400 to-rose-400 px-4 py-3.5 font-display text-base font-bold text-slate-950 shadow-glow disabled:cursor-not-allowed disabled:opacity-60 sm:rounded-[22px] sm:py-4 sm:text-lg">
        ...
        <div className="hidden space-y-3 sm:space-y-5 xl:block">
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.28fr)_minmax(18rem,0.94fr)] 2xl:gap-6">
          <section className="min-w-0 space-y-4 sm:space-y-6">
            ...
            <div className="grid gap-4 min-[460px]:grid-cols-2 2xl:grid-cols-3">
              {menuQuery.isLoading ? Array.from({ length: 6 }).map((_, index) => <SkeletonCard key={index} />) : null}
          ...
          <aside className="min-w-0 space-y-4 sm:space-y-6 xl:sticky xl:top-4 xl:self-start">
            <section className="rounded-[28px] border border-white/10 bg-white/5 p-4 backdrop-blur-xl sm:rounded-[32px] sm:p-5">
...
export const CustomerOrderDetailPage = () => {
  ...
  if (orderHistoryQuery.error || orderMenuItemsQuery.error) {
    return (
      <HostedMaintenancePanel
        title="وردەکاری داواکاری بارنابێت"
        message={getQueryErrorMessage(orderHistoryQuery.error ?? orderMenuItemsQuery.error, "ناتوانرێت وردەکاری داواکارییەکە ئێستا باربکرێت.")}
        action={
          <button type="button" onClick={() => window.location.reload()} className="compact-pill-button justify-center">
            نوێکردنەوە
          </button>
...
export const CustomerSettingsPage = () => {
  ...
  const customerAvailability = useCustomerAvailabilitySettings({ categories, menuItems });
  ...
  if (catalogQuery.error) {
    return (
      <HostedMaintenancePanel
        title="ڕێکخستنەکان بارنابن"
        message={getQueryErrorMessage(catalogQuery.error, "ناتوانرێت لیستی خواردن و پۆلەکان ئێستا باربکرێت.")}
        action={
          <button type="button" onClick={() => window.location.reload()} className="compact-pill-button justify-center">
            نوێکردنەوە
          </button>
  ...
  const hasDisabledAvailability = disabledCategoriesCount > 0 || disabledMenuItemsCount > 0;
    ...
    <div className="app-shell">
      <div className="app-stage space-y-4 sm:space-y-6">
        <CustomerPageHeader
          ...
          onBack={() => navigate("/app")}
          action={hasDisabledAvailability ? (
            <button
              type="button"
              onClick={customerAvailability.resetAllAvailability}
              className="compact-pill-button w-full justify-center sm:w-auto"
            >
              گەڕاندنەوەی هەموو پۆل و خواردن
            </button>
          ) : null}
        />
...
export const CustomerCategorySettingsDetailPage = () => {
  ...
  if (catalogQuery.error) {
    return (
      <HostedMaintenancePanel
        title="وردەکاری پۆل بارنابێت"
        message={getQueryErrorMessage(catalogQuery.error, "ناتوانرێت وردەکاری پۆلەکە ئێستا باربکرێت.")}
        action={
          <button type="button" onClick={() => window.location.reload()} className="compact-pill-button justify-center">
            نوێکردنەوە
          </button>
...
export const CustomerMenuItemSettingsDetailPage = () => {
  ...
  const { i18n } = useTranslation();
  const locale = i18n.language as Locale;
  const { catalogQuery, categories, menuItems, menuItemsById } = useCustomerSettingsCatalog(locale);
  const customerAvailability = useCustomerAvailabilitySettings({ categories, menuItems });
  ...
  if (catalogQuery.error) {
    return (
      <HostedMaintenancePanel
        title="وردەکاری خواردن بارنابێت"
        message={getQueryErrorMessage(catalogQuery.error, "ناتوانرێت وردەکاری خواردنەکە ئێستا باربکرێت.")}
        action={
          <button type="button" onClick={() => window.location.reload()} className="compact-pill-button justify-center">
            نوێکردنەوە
          </button>
