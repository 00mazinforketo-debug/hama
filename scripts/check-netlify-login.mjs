...
const fallbackSiteUrl = "https://98ramyar.netlify.app";
const siteUrl = (
  process.argv[2] ||
...
const inferIssues = (checks) => {
  ...
  const allChecks = [proxiedReady, directReady, proxiedLogin, directLogin];

  if (allChecks.some((check) => check.status === 0)) {
    issues.push({
      code: "site_unreachable_or_fetch_failed",
      severity: "high",
      message: "At least one production probe failed before any HTTP response was received, so live Netlify health could not be verified from this environment.",
      likelyFiles: [],
      recommendedFix: "Verify the Netlify site URL, DNS reachability, and whether this runtime can reach the live site before treating production as healthy."
    });
    return issues;
  }
  ...
  if (directReady.status === 404 && directLogin.status === 404) {
    issues.push({
      code: "direct_function_not_deployed",
      severity: "high",
      message: "The direct Netlify Function endpoints return 404, which usually means the function bundle is not deployed or the site is serving the wrong publish output.",
      likelyFiles: [
        "netlify.toml",
        "netlify/functions/api.ts",
        "apps/web/public/_redirects"
      ],
      recommendedFix: "Confirm the correct Netlify site is deployed, ensure the function bundle is included, and verify the publish directory contains the expected redirects file."
    });
  }
